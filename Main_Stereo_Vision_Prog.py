...
def coords_mouse_disp(event,x,y,flags,param):
    if event == cv2.EVENT_LBUTTONDBLCLK:
        ...
        average=average/9
        Distance= 5.1 * 10.16 / average                             #-593.97*average**(3) + 1506.8*average**(2) - 1373.1*average + 522.06
        Distance= np.around(Distance*0.01,decimals=2)
        print('Distance: '+ str(Distance*3.28084)+' ft')
...
cam= cv2.VideoCapture(gstreamer_pipeline(flip_method=0), cv2.CAP_GSTREAMER)   # Wenn 0 then Right Cam and wenn 2 Left Cam
